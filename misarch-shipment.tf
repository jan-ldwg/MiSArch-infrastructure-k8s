module "misarch_shipment" {
  ...
  service_name        = local.misarch_shipment_service_name
  image               = "ghcr.io/misarch/shipment:${var.MISARCH_SHIPMENT_VERSION}"
  namespace           = local.namespace
  base_configmap      = local.misarch_base_env_vars_configmap
  service_configmap   = local.misarch_shipment_env_vars_configmap
  ecs_configmap       = local.misarch_shipment_ecs_env_vars_configmap
  ...
  specific_labels     = local.misarch_shipment_specific_labels
  base_annotations    = local.base_misarch_annotations
  specific_annotations = local.misarch_shipment_specific_annotations
}
