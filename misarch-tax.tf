module "misarch_tax" {
  ...
  service_name        = local.misarch_tax_service_name
  image               = "ghcr.io/misarch/tax:${var.MISARCH_TAX_VERSION}"
  namespace           = local.namespace
  base_configmap      = local.misarch_base_env_vars_configmap
  service_configmap   = local.misarch_tax_env_vars_configmap
  ecs_configmap       = local.misarch_tax_ecs_env_vars_configmap
  ...
  specific_labels     = local.misarch_tax_specific_labels
  base_annotations    = local.base_misarch_annotations
  specific_annotations = local.misarch_tax_specific_annotations
}
